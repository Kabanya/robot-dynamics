# taken from
# https://gepettoweb.laas.fr/doc/stack-of-tasks/pinocchio/master/doxygen-html/md_doc_d_practical_exercises_6_wpg.html
# a bit modified
...
class FootSteps:
    ...
    def __init__(self, right, left):
        ...
        self.left = [left]
        self.timetime = [0.0]
        self.flying_foot = []  # добавлено: инициализация flying_foot
    ...
    def add_phase(self, duration, foot, position=None):
        ...
        self.timetime.append(self.timetime[-1] + duration)
    ...
    def get_index_from_time(self, t):
        ...
        """
        if t >= self.timetime[-1]:
            return len(self.timetime) - 2
        return next(i for i, ti in enumerate(self.timetime) if ti > t) - 1
    ...
    def get_left_next_position(self, t):
        i = self.get_index_from_time(t)
        i = i + 1 if i + 1 < len(self.timetime) else i
        return self.left[i]
    ...
    def get_phase_start(self, t):
        return self.timetime[self.get_index_from_time(t)]

    def get_phase_duration(self, t):
        i = self.get_index_from_time(t)
        return self.timetime[i + 1] - self.timetime[i]

    def get_phase_remaining(self, t):
        return self.timetime[self.get_index_from_time(t) + 1] - t